...
const { periksaUntukPinterest } = require('./linkcheck');
...
function buildMetadata({ platform, title = '', link = '', boardId = '', settings = {} }) {
  ...
  if (platform === 'pinterest') {
    ...
    // Link tujuan dicegat di sini. Pinterest memblokir link pendek dan
    // menolaknya dengan pesan "Unknown error" yang tidak menjelaskan apa pun,
    // jadi lebih baik ketahuan sebelum kuota terbakar.
    const { blokir, peringatan } = periksaUntukPinterest(link);
    if (blokir) missing.push(blokir);

    const pinterest = { boardServiceId: board };
    if (title.trim()) pinterest.title = title.trim().slice(0, 100);
    if (link.trim() && !blokir) pinterest.url = link.trim();
    return { metadata: { pinterest }, missing, peringatan };
  }
...
function buildPost({ platform, title = '', caption = '', hashtags = [], link = '', boardId = '', settings = {} }) {
  ...
  const { metadata, missing, peringatan } = buildMetadata({ platform, title, link, boardId, settings });
  const panjang = lengthWarning(platform, text);
  const warning = [panjang, peringatan].filter(Boolean).join(' ') || null;
  return { text, metadata, missing, warning };
}
