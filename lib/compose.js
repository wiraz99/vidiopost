...
const FACEBOOK_TYPE = process.env.FACEBOOK_POST_TYPE || 'reel';
...
function buildMetadata({ platform, title = '', link = '', boardId = '' }) {
  ...
  if (platform === 'facebook') {
    return { metadata: { facebook: { type: FACEBOOK_TYPE } }, missing };
  }
  ...
  if (platform === 'pinterest') {
    ...
    if (!board) missing.push('board Pinterest belum dipilih — pilih di halaman Jadwal, bagian Channel tujuan');
