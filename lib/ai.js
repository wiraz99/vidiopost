...
// Lapisan paling bawah saja. Brand yang sebenarnya dipakai datang dari GRUP
// (lihat lib/groups.js): satu deployment bisa melayani beberapa brand, dan
// kalau nama brand tetap konstanta modul, caption brand kedua akan menyebut
// brand pertama. Nilai di sini cuma dipakai kalau grupnya tidak mengisinya.
...
const context = (brief, { brand, product } = {}) =>
  `Brand: ${brand || BRAND}. Produk: ${product || PRODUCT}. Bahasa: Indonesia santai, ramah, tidak kaku.\n` +
  `Konteks video: "${brief}".`;

/** Caption per platform. Mempertahankan bentuk balasan endpoint /api/caption yang lama. */
async function generateCaptions({ brief, platforms, title = '', brand, product }) {
  if (!platforms?.length) throw new AIError('platforms kosong', 400);

  const prompt = `${context(brief, { brand, product })}
...
async function suggestTitles({ brief, count = 5, platform = 'youtube', brand, product }) {
  const prompt = `${context(brief, { brand, product })}
...
async function suggestHashtags({ brief, platform = 'instagram', count = 15, brand, product }) {
  const prompt = `${context(brief, { brand, product })}

Usulkan ${count} hashtag relevan untuk ${platform}.
Syarat:
- campur hashtag populer dan hashtag khusus/niche
- relevan dengan "${product || PRODUCT}" dan pasar Indonesia
