...
async function channelBoards(account, channelId, { force = false } = {}) {
  ...
  if (!force && entry?.boards?.length && Date.now() - new Date(entry.fetchedAt).getTime() < BOARDS_CACHE_TTL_MS) {
    return { boards: entry.boards, channelAda: true };
  }
  ...
  // dua keadaan itu butuh tindakan yang berbeda, dan kalau disamakan aplikasi
  // menyuruh user menyambung ulang padahal itu justru penyebabnya.
  const channelAda = !!data?.channel;
...
async function introspectMetrics(account = 'A') {
  const akun = TOKENS[account] ? account : (TOKENS.A ? 'A' : 'B');
