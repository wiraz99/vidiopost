...
async function discoverChannels({ force = false } = {}) {
  ...
  const akunSekarang = daftarAkun();

  // Akun mana saja yang sudah pernah ditanyai waktu cache ini dibuat. Cache
  // lama belum menyimpannya, jadi disimpulkan dari channel yang ada — sekali
  // saja, sesudah itu tersimpan apa adanya.
  const akunDiCache = cached?.akun || [...new Set((cached?.channels || []).map((c) => c.account))];

  // Token baru ditambahkan = daftar channel yang kita pegang PASTI basi.
  //
  // Cache bertahan 1 jam DAN ikut tersimpan di volume permanen, jadi deploy
  ...
  // berlalu atau tombol "Muat ulang channel" ditekan — dan tidak ada satu pun
  // petunjuk di layar kenapa channel barunya belum muncul.
  //
  // Yang dibandingkan adalah akun yang DITANYAI, bukan akun yang menghasilkan
  // channel. Akun yang tokennya benar tapi belum punya channel sama sekali
  // akan selamanya terlihat "baru", dan itu berarti memaksa ambil ulang di
  // setiap panggilan — kuota harian habis percuma.
  const adaAkunBaru = akunSekarang.some((a) => !akunDiCache.includes(a));

  if (!force && !adaAkunBaru && cached && Date.now() - new Date(cached.fetchedAt).getTime() < CHANNEL_CACHE_TTL_MS) {
    return {
      channels: cached.channels,
      source: 'cache',
      fetchedAt: cached.fetchedAt,
      // Kegagalan per akun ikut disimpan, supaya alasannya tidak menghilang
      // begitu jawabannya datang dari cache.
  ...
  // `akun` dicatat supaya penambahan token berikutnya langsung terdeteksi,
  // dan `errors` supaya kegagalan satu akun tetap bisa dilaporkan walau
  // jawabannya nanti datang dari cache.
  const fetchedAt = new Date().toISOString();
  store.write('channels-cache', { fetchedAt, channels: found, akun: akunSekarang, errors });
