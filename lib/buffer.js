...
const SCHEDULED_QUERY = `
  query Scheduled($organizationId: String!, $first: Int!) {
...
/** Jumlah post yang sedang mengantre, per channelId. Dibaca langsung dari Buffer. */
 ...
 */
async function createPost({ account, channelId, text, videoUrl, dueAt, mode = 'addToQueue' }) {
  const input = {
    text,
    channelId,
...
const SENT_WITH_METRICS = `
  query SentWithMetrics($organizationId: String!, $first: Int!) {
    posts(first: $first, input: {
      organizationId: $organizationId
      filter: { status: [sent] }
    }) {
