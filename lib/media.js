...
async function checkPublicUrl(filename, { force = false } = {}) {
  ...
  if (!filename) {
    ...
    try {
      ...
      // Terjemahkan error jaringan yang sering muncul jadi bahasa manusia.
      // "getaddrinfo ENOTFOUND" tidak berarti apa-apa buat kebanyakan orang.
      const raw = err.message || '';
      let reason;
      if (/ENOTFOUND|EAI_AGAIN/i.test(raw)) {
        const domain = raw.match(/ENOTFOUND ([^\s]+)/)?.[1] || new URL(url).hostname;
        reason = `Domain "${domain}" tidak dikenali sama sekali (tidak bisa diterjemahkan jadi alamat IP). ` +
          'Domainnya salah ketik, sudah kedaluwarsa, atau memang bukan alamat aplikasi ini.';
      } else if (/ECONNREFUSED/i.test(raw)) {
        reason = `Koneksi ke ${url} ditolak. Alamatnya ada tapi tidak ada yang melayani di sana.`;
      } else if (/certificate|SSL|TLS/i.test(raw)) {
        reason = `Sertifikat SSL bermasalah di ${url}. Buffer menolak mengunduh dari HTTPS yang tidak sah.`;
      } else if (/timeout|ETIMEDOUT/i.test(raw)) {
        reason = `${url} tidak menjawab (timeout). Kemungkinan terhalang firewall.`;
      } else {
        reason = `Tidak bisa menghubungi ${url} — ${raw}`;
      }
      result = { ...base, ok: false, reason, rawError: raw };
    }
  }
